...
/**
 * Priority-aware LLM inference service using the Ollama HTTP API.
 *
 * <p>Same functionality as {@link InferenceService}, but extends
 * {@link RPriorityService} to enforce bounded concurrency with separate
 * pools for remote and local requests.
 *
 * <p>Default configuration: 1 remote thread, 1 local thread, no queuing.
 * This means at most 1 remote inference and 1 local inference can run
 * concurrently; additional requests are rejected immediately.
 */
@Streamable
public class InferencePriorityService extends RPriorityService {
    ...
    public InferencePriorityService(String endpoint, String model, Config config) {
        super(config);
